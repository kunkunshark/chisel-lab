...
class VendingTest4 extends AnyFlatSpec with ChiselScalatestTester {
  "Vending machine test" should "pass" in {
    test(new VendingMachine(20)).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
      println("This test block expects the machine to successfully output \"alarm\" to avoid overflow (for add2, when sum < 30)")
      dut.io.price.poke(30.U) // set price to 30
      dut.clock.step(2)

      for (add30 <- 0 to 15) { // add to 30 from coin2
